...
class MLWHDiffDB(CacheDB):
    """
    Maintains a cache of differences between the MLWH and ToLQC.  Differences,
    identified by their `data_id` key can be labelled with reasons.
    """
    ...
    def store_reasons(self, reason, data_id_list):
        sql = """
          ...
          FROM
            (
              SELECT
                unnest(list_transform(?, lambda x: (x, ?)), recursive := true)
            )
        """
    ...
    def create_diff_reason_table(self):
        self.execute("""
    ...
    def create_or_update_macros_and_views(self):
        ...
        for sql_def in (
            ...
            self.execute(" " * 14 + "CREATE OR REPLACE " + sql_def)
